...
export const Statistics = ({
  ...
  const { state, flippedcardarray ,truevaluearray,compflippedcardarray,comptruevaluearray,averageresponsetime,averageresponsetimecomp} = useContext(GameContext);
...
{/* _____________________________ALL CARD STAT_________________________________ */}
      ...
      <Modal show={carddetail} className="all_card_stat">
        ...
        <Modal.Body>
          <div className="wrap_stat carties_table " style={{ overflowX: "auto",display: "flex" }}>
            <table className="table table-striped">
            <thead> 
              <h4>{userName}</h4></thead>
  <tbody>
              <tr>
                <th scope="row">VALUE</th>
                <th scope="row" style={{ textAlign: 'center' }}>NOMBRE DE FOIS RETOURNÉS</th>
                <th scope="row" style={{ textAlign: 'center' }}>NOMBRE DE BONNES RÉPONSES</th>
                <th scope="row" style={{ textAlign: 'center' }}>TEMPS DE RÉPONSE MOYEN</th>
                <th scope="row" style={{ textAlign: 'center' }}>TAUX %</th>
              </tr>
            ...
            const averageData = averageresponsetime[cardName] || { sum: 0, count: 0, average: 0 };
            ...
            return (
              <tr key={cardName}>
                ...
                <td style={{ textAlign: 'center' }}>{(averageData.average/1000).toFixed(2)}</td>
...
<tr>
    ...
    <td style={{ textAlign: 'center' }}>
  {(Object.values(averageresponsetime).reduce((acc, data) => (acc + data.average), 0) / 1000).toFixed(2)}
</td>
...
<tbody>
          ...
          const ratePercentage = (rightAnswerCount / timesReturned) * 100 || 0;
          const averageDataComp = averageresponsetimecomp[cardName] || { sum: 0, count: 0, average: 0 };
          ...
          return (
            <tr key={cardName}>
              ...
              <td style={{ textAlign: 'center' }}>{timesReturned}</td>
              <td style={{ textAlign: 'center' }}>{rightAnswerCount}</td>
              <td style={{ textAlign: 'center' }}>{(averageDataComp.average/1000).toFixed(2)}</td>
              <td style={{ textAlign: 'center' }}>{ratePercentage.toFixed(0)}%</td>
            </tr>
          );
        })}
...
<tr>
  <th scope="row">Total :</th>
  <td style={{ textAlign: 'center' }}>
    {Object.values(cardCountsComp).reduce((acc, count) => acc + count, 0)}
  </td>
  <td style={{ textAlign: 'center' }}>
    {Object.values(rightAnswersComp).reduce((acc, count) => acc + count, 0)}
  </td>
  <td style={{ textAlign: 'center' }}>
  {(Object.values(averageresponsetimecomp).reduce((acc, data) => (acc + data.average), 0) / 1000).toFixed(2)}
</td>
  <td style={{ textAlign: 'center' }}>
  {Math.round(
  (Object.values(rightAnswersComp).reduce((acc, count) => acc + count, 0) /
